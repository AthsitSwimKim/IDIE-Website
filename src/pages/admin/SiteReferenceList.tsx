...
export default function AdminSiteReferenceList() {
  ...
  const remove = useDeleteConfirm<AdminSiteReference>(
    (item) => adminSiteReferences.remove(item.id),
  ...
  return (
    <>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    ...
                    */}
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="sm" to={`/admin/site-references/${item.id}`}>
                        แก้ไข
                      </Button>
                      <Button
                        variant="ghost"
      ...
      >
        <p>
          “{remove.target?.name.th}” จะถูกลบออกจากระบบถาวร และหายจากหน้าลูกค้าอ้างอิงทันที
        </p>
        <p>ไฟล์รูปที่ไม่มีรายการอื่นใช้งานจะถูกลบถาวรด้วย รูปที่ใช้ร่วมกับรายการอื่นจะยังอยู่</p>
